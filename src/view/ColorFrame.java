...
public class ColorFrame extends JFrame {
    ...
    public ColorFrame(String s) {
        ...
        btn.addActionListener((e) -> {
            CellComponent[][] cellcomponent = gameFrame.getChessboardComponent().gridComponents;
            ...
            if (s.equals("Grass")){
                for (int i = 0; i < 9; i++) {
                    for (int j = 0; j < 7; j++) {
                        if (cellcomponent[i][j].type == CellType.ORIGIN_GRASS
                            ...
                            cellcomponent[i][j].type = CellType.OTHER_GRASS;
                            cellcomponent[i][j].background = c;
                            cellcomponent[i][j].repaint();
                            cellcomponent[i][j].revalidate();
                        }
                    }
                }
                ...
                for (int i = 0; i < 9; i++) {
                    for (int j = 0; j < 7; j++) {
                        if (cellcomponent[i][j].type == CellType.TRAP || cellcomponent[i][j].type == CellType.OTHER_TRAP){
                            cellcomponent[i][j].type = CellType.OTHER_TRAP;
                            cellcomponent[i][j].background = c;
                            cellcomponent[i][j].repaint();
                            cellcomponent[i][j].revalidate();
                        }
                ...
                for (int i = 0; i < 9; i++) {
                    for (int j = 0; j < 7; j++) {
                        if (cellcomponent[i][j].type == CellType.DEN || cellcomponent[i][j].type == CellType.OTHER_DEN){
                            cellcomponent[i][j].type = CellType.OTHER_DEN;
                            cellcomponent[i][j].background = c;
                            cellcomponent[i][j].repaint();
                            cellcomponent[i][j].revalidate();
                        }
            ...
            gameFrame.getChessboardComponent().repaint();
            gameFrame.getChessboardComponent().revalidate();
            this.dispose();
        });
        init();
